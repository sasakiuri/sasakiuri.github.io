...
- `Update diary`
  で毎日00時23分（日本時間）に FxEmbed の無料・認証不要 API から X の公開投稿を取得します。変更時だけ Git に保存し、検証済み配信を開始します。
- `pnpm diary:update --dry-run` で追加件数と本文更新件数を確認し、`pnpm diary:update`
  で保存します。最新投稿が保存済みでも、取得範囲内の欠落投稿を補完します。
- 日記更新が失敗しても公開済みデータは維持されます。HTTP
  429・5xx と通信失敗は各ページ最大3回試行します。JSON 形式、投稿者、投稿日時、カーソル、本文の整合性エラーでは保存を中止します。FxEmbed の仕様変更時は抽出 fixture と検証処理を更新します。
- 保存済みの投稿が続くページか終端に最大10ページで到達できない場合も保存を中止します。長期間の停止による欠落はブラウザーから書き出して取り込みます。ローカルでの取得成功だけでは、Actions の自動更新が復旧したとは判断しません。
- 2026年9月の停止原因は Actions から X への直接取得に対する HTTP
  403 でした。FxEmbed に切り替えたため、通常更新は公開 HTML の構造や取得可否に依存しません。
- Actions の取得環境を比較する場合は、`Update diary` の手動実行で `diagnose`
  を有効にします。Ubuntu と macOS で公開プロフィール、旧ドメイン、埋め込み用エンドポイント、FxEmbed、Chromium の応答を記録します。診断ジョブは読み取り権限だけを持ち、通常の更新・公開ジョブは実行しません。診断完了は取得成功を意味しません。各ログの HTTP
  status、抽出エラー、投稿 ID と日時を確認します。
