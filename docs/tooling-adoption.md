...
`pnpm deps:audit` は失敗します。OSV の例外も同じ日に失効します。GitHub Dependency
Review は同じ 2 件の ID を許可しますが、同じ workflow 内の `pnpm deps:audit`
が依存経路と期限を強制するため、期限後に品質ゲートを通過することはありません。
